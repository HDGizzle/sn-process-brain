...
| **Q10** three active BRs whose `ohs_task.category=qrt` filter matches zero rows | QS-05a | no |
...
> **Worked, real the pilot customer data.** `sn_ohs_im`, table `sn_ohs_im_action`, three active business rules `9ec847db…35b8`, `c75a1f57…35d4`, `d01a1f17…35f3`, all carrying `filter_condition: ohs_task.category=qrt`. `ohs_task` references base `sn_ohs_im_task`; `category` exists only on child `sn_ohs_im_incident`. ServiceNow drops the clause and returns unfiltered rows, so the rules match zero rows and have never fired. Cluster of 3, **one** question. Sources `wiki/conventions.md:60-82`, `audit-phase0-findings.md:39`, `_lab/INTERVIEW.md:85-90`. **Real outcome: answered in one line ("Sherwin is building the fix, 1267573"), logged as TBD-040 KNOWN-BROKEN.**
